#!/usr/bin/env python3
"""Fast Dataset Generator for Cryptographic Algorithm Identification"""
...
def encrypt_aes(plaintext):
    ...
    key = os.urandom(random.choice([16, 24, 32]))
    iv = os.urandom(16)
...
def encrypt_blowfish(plaintext):
    ...
    from Crypto.Util.Padding import pad
    key = os.urandom(random.randint(16, 56))
    iv = os.urandom(8)
...
def encrypt_rsa(plaintext):
    ...
    key = RSA.generate(1024)  # Smaller key for speed
...
FUNCS = {
    'AES': encrypt_aes, 'DES': encrypt_des, '3DES': encrypt_3des,
    'Blowfish': encrypt_blowfish, 'RSA': encrypt_rsa,
    'MD5': hash_md5, 'SHA1': hash_sha1, 'SHA256': hash_sha256,
    'Base64': encode_base64, 'Hex': encode_hex
}
